...
namespace AIStudio.Service.AppClient
{
    public class DataProvider
    {
        ...
        public DataProvider(string url, string userName, string password, int headMode)
         : this(url, userName, password, headMode, TimeSpan.FromSeconds(8))
        {
        }
        ...
        public async Task<WebResponse<string>> GetToken()
        {
            try
            {
                ...
                {
                    return WebResponse<string>.Success(response.Data as string);
                }
        ...
        public async Task<WebResponse<T>> GetData<T>(string url)
        {
            try
            {
                var response = await _transfer.GetData(url);
                if (response.Success == true)
                {
                    return WebResponse<T>.Success(JsonConvert.DeserializeObject<T>((response.Data ?? "").ToString()));
                }
        ...
        public async Task<WebResponse<List<T>>> Query<T>(ICollection<string> columns, string condition, object[] args, CompressionType zip = CompressionType.None)
        {
            try
            {
                ...
                if (response.Success == true)
                {
                    return WebResponse<List<T>>.Success(JsonConvert.DeserializeObject<List<T>>((response.Data ?? "").ToString()));
                }
            ...
            try
            {
                ...
                if (response.Success == true)
                {
                    return WebResponse<IDictionary<string, ICollection<object>>>.Success(
                        JsonConvert.DeserializeObject<IDictionary<string, ICollection<object>>>((response.Data ?? "").ToString()));
                }
            ...
            ICollection<ComplexQuery> queries, CompressionType zip = CompressionType.None)
        {
            try
            {
                ...
                if (response.Success == true)
                {
                    ...
                    return WebResponse<ComplexQueryResult>.Success(result);
                }
