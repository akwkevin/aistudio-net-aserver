...
export default {
  ...
  onMouseup: function (e) {
    ...
    if (item && item.getType() === 'node') {
      ...
      if (
        typeof e.target.attrs.isInPoint !== 'undefined' &&
        e.target.attrs.isInPoint
      ) {
        ...
        typeof e.target.attrs.isInPointOut !== 'undefined' &&
        e.target.attrs.isInPointOut
      ) {
        activeItem = e.target
        const children = group.cfg.children
        children.map((child) => {
          if (
            typeof child.attrs.isInPoint !== 'undefined' &&
    ...
    this.graph.find('node', (node) => {
      ...
      children.map((child) => {
        if (
          typeof child.attrs.isInPointOut !== 'undefined' &&
          child.attrs.isInPointOut
        ) {
        ...
        if (
          typeof child.attrs.isOutPoint !== 'undefined' &&
          child.attrs.isOutPoint
        ) {
          child.attr('opacity', '0')
          child.attr('fill', '#fff')
        }
